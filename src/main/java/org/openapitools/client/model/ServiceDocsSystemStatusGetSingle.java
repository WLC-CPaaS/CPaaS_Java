  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ServiceDocsSystemStatusGetSingle.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ServiceDocsSystemStatusGetSingle is not found in the empty JSON string", ServiceDocsSystemStatusGetSingle.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ServiceDocsSystemStatusGetSingle.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ServiceDocsSystemStatusGetSingle` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("system_status") != null && !jsonObj.get("system_status").isJsonNull()) && !jsonObj.get("system_status").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "Expected the field `system_status` to be a primitive type in the JSON string but got `%s`", jsonObj.get("system_status").toString()));
      }
  }
